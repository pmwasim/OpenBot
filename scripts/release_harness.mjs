...
async function main() {
  ...
  try {
    ...
    try {
      ...
      const resultView = await http(`/api/tasks/${encodeURIComponent(asyncCreateBody.task.id)}/result`);
      const resultBody = JSON.parse(resultView.body);
      if (resultView.status !== 200 || !String(resultView.headers['content-type']).includes('application/json') || resultBody.taskId !== asyncCreateBody.task.id || resultBody.status !== 'completed' || resultBody.result !== 'The asynchronous task completed.' || !Array.isArray(resultBody.actions) || Object.hasOwn(resultBody, 'events')) throw new Error(`task result view ${resultView.status} ${resultView.body}`);
      pass('completed task results have a concise structured view separate from the full audit');
