...
import { CONNECTOR_LIMITS, connectorView, validateConnectorDefinition } from './connectors.mjs';
...
export const BOT_LIMITS = Object.freeze({ maxBots: 50, maxMessages: 40, maxQueryChars: 120 });
export const TASK_RETRY_LIMITS = Object.freeze({ maxRetries: 3 });
...
function emptyProjection() {
  return { events: [], tasks: new Map(), approvals: new Map(), routines: new Map(), bots: new Map(), connectors: new Map(), actions: new Map(), memories: new Map(), skills: new Map(), seq: 0 };
}
  ...
  switch (event.type) {
    case 'task.created': {
      const task = {
        ...
        retryCount: Number(payload.retryCount) > 0 ? Number(payload.retryCount) : 0,
        queueState: payload.queueState || 'idle'
      };
    ...
    case 'task.released': {
      const task = projection.tasks.get(event.taskId);
      if (task) {
        task.queueState = payload.queueState || (event.type === 'task.queued' ? 'queued' : event.type === 'task.admitted' ? 'running' : 'idle');
        task.updatedAt = event.ts;
        task.eventSequence = event.seq;
      }
      break;
    ...
    case 'bot.deleted':
      ...
      break;
    case 'connector.seeded':
    case 'connector.created':
    case 'connector.updated': {
      if (!payload.id || !payload.name || !payload.baseUrl || !Array.isArray(payload.allowedPaths)) break;
      const previous = projection.connectors.get(payload.id) || {};
      projection.connectors.set(payload.id, {
        ...previous,
        ...connectorView({ ...payload, createdAt: previous.createdAt || event.ts, updatedAt: event.ts })
      });
      break;
    }
    case 'connector.deleted':
      if (payload.id) projection.connectors.delete(payload.id);
      break;
    case 'bot.message': {
...
export async function openStore(options = {}) {
  ...
  async function seedFrom(state) {
    ...
    for (const connector of state.connectors || []) {
      await appendUnlocked({ type: 'connector.seeded', payload: { ...connector } });
    }
  }
  ...
  async function appendUnlocked(partial) {
    ...
    await persist();
    for (const listener of taskSubscribers.get(event.taskId) || []) {
      try { listener(event); } catch { /* stream clients cannot affect durable writes */ }
    }
    return event;
  }
  ...
  return {
    ...
    subscribeTaskEvents(taskId, listener) {
      ...
      return () => {
        ...
        if (!listeners.size) taskSubscribers.delete(taskId);
      };
    },
    async getState() {
    ...
    async listBotMessages(id, { query = '' } = {}) {
      return withLock(async () => {
        const bot = projection.bots.get(id);
        if (!bot) return null;
        const normalizedQuery = String(query || '').trim();
        if (normalizedQuery.length > BOT_LIMITS.maxQueryChars) throw httpError(`Conversation search must be ${BOT_LIMITS.maxQueryChars} characters or fewer.`, 400);
        const needle = normalizedQuery.toLocaleLowerCase();
        return (bot.messages || []).slice(-BOT_LIMITS.maxMessages).filter((message) => !needle || String(message.content || '').toLocaleLowerCase().includes(needle)).map((message) => ({
          id: message.id,
          role: message.role,
          content: redactSecrets(String(message.content || '')).slice(0, 12000),
    ...
    async listConnectors() {
      return withLock(async () => [...projection.connectors.values()]
        .sort((a, b) => String(a.name).localeCompare(String(b.name)))
        .map((connector) => connectorView(connector)));
    },
    async getConnector(id) {
      return withLock(async () => {
        const connector = projection.connectors.get(String(id || ''));
        return connector ? connectorView(connector) : null;
      });
    },
    async createConnector(input = {}) {
      return withLock(async () => {
        if (projection.connectors.size >= CONNECTOR_LIMITS.maxConnectors) throw httpError(`OpenBot supports at most ${CONNECTOR_LIMITS.maxConnectors} connectors.`, 409);
        const id = input.id || `connector-${randomUUID()}`;
        const definition = validateConnectorDefinition({ ...input, id, owner: input.owner || 'operator' });
        if ([...projection.connectors.values()].some((connector) => connector.name.toLowerCase() === definition.name.toLowerCase())) throw httpError('A connector with that name already exists.', 409);
        await appendUnlocked({ type: 'connector.created', actor: input.owner || 'operator', payload: definition });
        return { connector: connectorView(projection.connectors.get(id)) };
      });
    },
    async updateConnector(id, patch = {}) {
      return withLock(async () => {
        const current = projection.connectors.get(id);
        if (!current) throw httpError('Connector not found.', 404);
        const next = validateConnectorDefinition({ ...current, ...patch, id, owner: current.owner });
        if ([...projection.connectors.values()].some((connector) => connector.id !== id && connector.name.toLowerCase() === next.name.toLowerCase())) throw httpError('A connector with that name already exists.', 409);
        await appendUnlocked({ type: 'connector.updated', actor: 'operator', payload: next });
        return { connector: connectorView(projection.connectors.get(id)) };
      });
    },
    async deleteConnector(id) {
      return withLock(async () => {
        if (!projection.connectors.has(id)) throw httpError('Connector not found.', 404);
        await appendUnlocked({ type: 'connector.deleted', actor: 'operator', payload: { id } });
    ...
    async recordBotMessages(id, inputs = []) {
      return withLock(async () => {
        if (!projection.bots.has(id)) throw httpError('Bot not found.', 404);
        if (!Array.isArray(inputs) || !inputs.length || inputs.length > 4) throw httpError('A bot message batch must contain between 1 and 4 messages.', 400);
        const messages = inputs.map((input = {}) => {
          const role = String(input.role || '').trim().toLowerCase();
          const content = redactSecrets(String(input.content || '').trim());
          if (!['user', 'assistant'].includes(role)) throw httpError('Bot message role must be user or assistant.', 400);
          if (!content || content.length > 12000) throw httpError('Bot message must be between 1 and 12000 characters.', 400);
          return { id: input.id || `message-${randomUUID()}`, role, content, taskId: input.taskId || null, createdAt: new Date().toISOString() };
        });
        for (const message of messages) await appendUnlocked({ type: 'bot.message', actor: 'agent', payload: { id, message } });
        return messages;
      });
    },
    async listRoutines() {
    ...
    async setTaskStatus(id, action) {
      return withLock(async () => {
        ...
        if (action === 'retry') {
          ...
          if (retryCount >= TASK_RETRY_LIMITS.maxRetries) throw httpError(`Task retry limit reached (${TASK_RETRY_LIMITS.maxRetries}).`, 409);
          await appendUnlocked({ type: 'task.retry', taskId: id, actor: 'operator', payload: { status: 'pending', action, retryCount: retryCount + 1, maxRetries: TASK_RETRY_LIMITS.maxRetries } });
