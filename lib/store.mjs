...
function emptyProjection() {
  return { events: [], tasks: new Map(), approvals: new Map(), routines: new Map(), actions: new Map(), seq: 0 };
}
...
export async function openStore(options = {}) {
  ...
  return {
    ...
    async consumeApproval({ id, argsDigest, actionId } = {}) {
      return withLock(async () => {
        const item = projection.approvals.get(id);
      ...
      return withLock(async () => {
        ...
        await appendUnlocked({
          ...
          payload: {
            ...
            boundArgs: input.boundArgs || null
          }
        });
