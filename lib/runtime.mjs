...
export async function proposeAction(store, input = {}) {
  const tool = mapTool(input.tool || input.kind);
  const workspace = await resolveWorkspace(store, input);
  const engine = createEngine({ store, actor: input.actor || 'operator' });
  return engine.act({
...
export async function executeAction(store, actionId, env = process.env) {
  ...
  const engine = createEngine({ store, actor: 'openbot' });
  return engine.act({
    actor: 'openbot',
