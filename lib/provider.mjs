...
export function createLocalModelAdapter({ baseUrl = 'http://127.0.0.1:11434', timeoutMs = 120000 } = {}) {
...
export function createRemoteCompatibleAdapter({ baseUrl = '', apiKey = '' } = {}) {
  ...
  return {
    ...
    async chat() {
      ...
      void apiKey;
      const error = new Error('Remote-compatible provider is a Phase 0 stub and is not implemented.');
      error.statusCode = 501;
      throw error;
    }
  };
...
export function createProviderHub(env = process.env, options = {}) {
  ...
  const remote = createRemoteCompatibleAdapter({
    baseUrl: options.remoteBaseUrl || env.OPENBOT_REMOTE_BASE_URL || '',
  ...
  return {
    ...
    describe() {
      return redactSecrets({
        localOnly,
        localModel: { id: localModel.id, local: true, enabled: true, baseUrl: localModel.baseUrl },
        remoteCompatible: {
          id: remote.id,
          local: false,
