...
export function createProviderHub(env = process.env, options = {}) {
  ...
  const ollamaUrl = options.ollamaUrl || env.OPENBOT_OLLAMA_URL || 'http://127.0.0.1:11434';
  let ollamaHost;
  try { ollamaHost = new URL(ollamaUrl).hostname; }
  catch { throw new Error('OPENBOT_OLLAMA_URL must be a valid http(s) URL.'); }
  if (localOnly && !['127.0.0.1', 'localhost', '::1'].includes(ollamaHost)) {
    const error = new Error('Local-only mode requires Ollama on loopback. Set OPENBOT_LOCAL_ONLY=0 to opt into a remote endpoint.');
