...
import { selectResourceProfile } from './config.mjs';

const execFileAsync = promisify(execFile);
let isolationCache = null;

export async function detectIsolation(env = process.env) {
  const requested = String(env.OPENBOT_ISOLATION || (selectResourceProfile(env).profile === 'legacy' ? 'cwd' : 'auto')).trim().toLowerCase();
  if (requested === 'cwd') {
    return { mode: 'cwd', image: null, reason: 'explicit local isolation' };
  }
