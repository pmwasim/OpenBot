...
export function daemonRetryTask(config, taskId, payload = {}, env = process.env) {
  return requestDaemon(config, taskPath(taskId, '/retry'), { method: 'POST', body: payload }, env);
}
...
export function daemonExportSkills(config, env = process.env) {
  return requestDaemon(config, '/api/skills/export', {}, env);
}

export function daemonImportSkills(config, pack, env = process.env) {
  return requestDaemon(config, '/api/skills/import', { method: 'POST', body: { pack } }, env);
}
