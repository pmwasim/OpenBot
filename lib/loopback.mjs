...
export function assertBindHost(host, env = process.env) {
  ...
  const error = new Error(
    `Refusing to bind HOST=${value}. OpenBot must stay on loopback by default. Set OPENBOT_ALLOW_NON_LOOPBACK=1 only with OPENBOT_AUTH_TOKEN for protected LAN mode.`
  );
...
export function hasBearerToken(request, expected) {
  if (!expected || !request?.headers) return false;
