...
async function dispatch(tool, args, workspace, signal) {
  ...
  if (tool === 'browser.fetch') {
    return browserFetch({ url: args.url, workspace, path: args.path || 'research.md', signal });
  }
...
export function createEngine({ store, actor = 'openbot' } = {}) {
  ...
  async function act(input = {}) {
    ...
    try {
      const output = await dispatch(tool, args, workspace, input.signal);
