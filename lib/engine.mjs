...
async function dispatch(tool, args, workspace, signal, browserAllowHosts) {
...
export function createEngine({ store, actor = 'openbot', browserAllowHosts } = {}) {
  ...
  async function act(input = {}) {
    ...
    const classified = classifyAction({ tool, args, workspace, browserAllowHosts });
    ...
    try {
      const output = await dispatch(tool, args, workspace, input.signal, browserAllowHosts);
