...
export function digestAction({ tool, args, workspace }) {
  return createHash('sha256').update(JSON.stringify(stable({ tool, args, workspace }))).digest('hex');
}
...
async function dispatch(tool, args, workspace) {
  ...
  if (tool === 'shell.exec') {
    const output = await shellExec(workspace, args.command);
  ...
  if (tool === 'browser.fetch') {
    return browserFetch({ url: args.url, workspace, path: args.path || 'research.md' });
  }
...
function auditFields(actor, tool, args, result) {
  ...
  return { actor, tool, args: safeArgs, result };
}

export function createEngine({ store, actor = 'openbot' } = {}) {
  ...
  async function act(input = {}) {
    ...
    const classified = classifyAction({ tool, args, workspace });
    const policy = decide(classified);
    const argsDigest = digestAction({ tool, args, workspace });
    ...
    try {
      const output = await dispatch(tool, args, workspace);
