...
export function classifyShellCommand(command, workspace) {
  ...
  if (program === 'node' && args.length === 1 && (args[0] === '-v' || args[0] === '--version')) {
  ...
  if (program === 'ls' && args.every((arg) => arg.startsWith('-') || !pathOutsideWorkspace(workspace, arg))) {
    return { safe: true, destructive: false, outsideWorkspace: false, deletes: false, argv };
  }
...
export function classifyBrowserUrl(url, { allowHosts = ['127.0.0.1', 'localhost'] } = {}) {
  ...
  if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
    return { allowlisted: false, reason: 'Only http(s) loopback URLs are allowed.', parsed };
  }
...
export function classifyAction(action = {}) {
  ...
  if (tool === 'browser.fetch' || tool === 'browser.save' || tool === 'browser.visit') {
    const classified = classifyBrowserUrl(args.url);
...
export function describePolicy() {
  return {
    default: 'allow',
    require_approval: [...REQUIRE_APPROVAL_KINDS, 'file.write', 'shell.delete'],
