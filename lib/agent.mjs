...
export function createAgentController({ store, provider, engine, actor = 'agent', maxTurns = 6, maxActions = 6, maxContextChars = 12000 } = {}) {
  ...
  async function run(input = {}) {
    ...
    const skillSelector = String(input.skill || taskRecord?.skill || '').trim();
    ...
    if (!taskId) {
      const created = await store.createTask({ prompt, kind: 'plan', provider: 'ollama', workspace, owner: actor, skill: selectedSkill?.id || null });
    ...
    const actions = [];
    const approvals = [];
    await event('agent.started', taskId, { promptChars: prompt.length, workspace, model: model || null, skill: selectedSkill?.name || null, maxTurns: turnLimit, maxActions: actionLimit });
