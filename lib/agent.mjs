...
export function createAgentController({ store, provider, engine, actor = 'agent', maxTurns = 6, maxActions = 6, maxContextChars = 12000 } = {}) {
  ...
  async function run(input = {}) {
    ...
    if (!taskId) {
      const created = await store.createTask({ prompt, kind: 'plan', provider: 'ollama', workspace, owner: actor });
      taskId = created.task.id;
    }
    const messages = [{ role: 'system', content: SYSTEM_PROMPT }, { role: 'user', content: prompt }];
    const actions = [];
    const approvals = [];
    await event('agent.started', taskId, { promptChars: prompt.length, workspace, model: model || null, maxTurns: turnLimit, maxActions: actionLimit });
    ...
    for (let turn = 1; turn <= turnLimit; turn += 1) {
      ...
      await event('model.request', taskId, { provider: 'ollama', model: model || null, turn, messageCount: modelMessages.length, contextChars: modelMessages.reduce((sum, item) => sum + String(item.content || '').length, 0) });
      let response;
      ...
      if (!response?.ok) {
        ...
        await event('model.failed', taskId, { provider: 'ollama', model: model || null, turn, error });
      ...
      await event('model.response', taskId, { provider: 'ollama', model: response.model || model || null, turn, responseChars: String(response.reply || '').length });
      let envelope;
      ...
      if (envelope.reply) {
        ...
        await event('agent.completed', taskId, { turn, actions: actions.length, replyChars: envelope.reply.length });
        await event('task.status', taskId, { status: 'completed', reason: 'reply' });
        return { taskId, status: 'completed', reply: envelope.reply, actions, approvals, turns: turn };
      }

      messages.push({ role: 'assistant', content: response.reply });
      const outcome = await engine.act({ actor, taskId, workspace, tool: envelope.action.tool, args: envelope.action.args, execute: true, prompt });
