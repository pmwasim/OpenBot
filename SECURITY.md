...
Workers remain policy-controlled and workspace-bound. File writes, fetch-and-save, and consequential actions require approval. Secrets and token-like content are redacted before agent context and persisted audit records. Report a suspected vulnerability through the repository's private security reporting flow when enabled; otherwise open a public issue without including secrets or exploit credentials.
