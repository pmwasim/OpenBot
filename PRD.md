...
This is not a claim of parity with a managed cloud service. Hosted computers, persistent shared environments, connectors, collaboration, streaming event clients, and an OS-installed service manager remain OpenBot roadmap items; OpenBot currently provides scoped operator-controlled local memory, explicit declarative local skills, local routines, a portable detached daemon, and opt-in CLI chat through that shared daemon while the host is active. OpenBot's differentiator is local ownership, zero mandatory spend, and inspectable policy/audit behavior.
