...
- Operators can register local read-only HTTP connectors through the API, dashboard, or CLI; connector hosts use the exact configured allowlist, paths are explicit, requests are GET-only with disabled redirects and bounded responses, and every `connector.fetch` action requires approval.
